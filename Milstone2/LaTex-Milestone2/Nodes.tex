...
\begin{scope}[every node/.style={right}]
    \path (Root) ++(5cm,0) node {$\Rightarrow$} ++(5mm,0) node {$1^{st}$ level};
    \path (j) ++(5mm,0) node {$\Rightarrow$} ++(5mm,0) node {$2^{nd}$ level};
    \path (l) ++(5mm,0) node {$\Rightarrow$} ++(5mm,0) node {$3^{rd}$ level};
    \path (l12) ++(5mm,0) node {$\Rightarrow$} ++(5mm,0) node {$(n-1)^{th}$ level};
    \path (merge3) ++(2cm,0) node {$\Rightarrow$} ++(5mm,0) node {$n^{th}$ level};
\end{scope}
